...
/*   Updated: 2023/10/12 12:36:51 by jomirand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_imgs_xpm(t_data *d)
{
	...
	d->e_img->img = mlx_xpm_file_to_image(d->mlx_ptr, d->map_utils->ea,
			&d->e_img->w, &d->e_img->h);
	if (!d->img->img || !d->minimap_img->img || !d->n_img->img
		|| !d->s_img->img || !d->w_img->img || !d->e_img->img)
	{
		ft_free_game4(d);
...
void	get_imgs_addr(t_data *d)
{
	...
	d->minimap_img->addr = mlx_get_data_addr(d->minimap_img->img,
			&d->minimap_img->bpp, &d->minimap_img->line_length,
			&d->minimap_img->endian);
